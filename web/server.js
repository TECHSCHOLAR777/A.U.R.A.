...
function getActivityId(candidate) {
  return candidate?.id || candidate?.activity_id || candidate?.node_id || 'activity';
}
...
function buildC1(candidate, validationResult, childProfiles, nodeId, offline) {
  ...
  // activity_id uses the bank id (activity_id/id) + nodeId + timestamp.
  const bankId = getActivityId(candidate);
  const activity_id = `${bankId}_${nodeId}_${Date.now()}`;
  ...
  return {
    ...
    provenance: {
      ...
      cache_key:         cacheKey(bankId, nodeId, age_band),
      fallback_tier:     'none', // primary
    },
  };
}

function buildAlternatives(candidates, childProfiles, nodeId, excludeId, limit = 2) {
  const alternatives = [];

  for (const candidate of candidates) {
    const candidateId = getActivityId(candidate);
    if (candidateId === excludeId) continue;

    const result = GuardrailEngine.validate(candidate, childProfiles);
    if (result.action === null || result.action === 'flag_modify') {
      alternatives.push(buildC1(candidate, result, childProfiles, nodeId, false));
    } else if (result.action === 'block_and_substitute') {
      const substitute = selectSafeSubstitute(activityBank, nodeId, candidateId);
      if (substitute) {
        const result2 = GuardrailEngine.validate(substitute, childProfiles);
        alternatives.push(buildC1(substitute, result2, childProfiles, nodeId, false));
      }
    }
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (result.action === null || result.action === 'flag_modify') {
        ...
        const alternatives = buildAlternatives(candidates, child_profiles, cleanNodeId, getActivityId(candidate));
        return res.status(200).json({ ...c1, alternatives });

      } else if (result.action === 'block_and_substitute') {
        // Substitute with a safe activity
        const substitute = selectSafeSubstitute(activityBank, cleanNodeId, getActivityId(candidate));
        if (substitute) {
          ...
          const alternatives = buildAlternatives(candidates, child_profiles, cleanNodeId, getActivityId(substitute));
          return res.status(200).json({ ...c1, alternatives });
        }
    ...
    return res.status(200).json({ ...fallback, alternatives: [] });
...
app.post('/api/llm/activity', (req, res) => {
  const { child_profiles, node_id, age_band_months, context, bandit_weights } = req.body || {};
  ...
  try {
    ...
    if (bandit_weights && context) {
      const features = extractFeatures(context);
      candidates = [...candidates].sort((a, b) => {
        const scoreA = predictReward(getActivityId(a), features, bandit_weights);
        const scoreB = predictReward(getActivityId(b), features, bandit_weights);
    ...
    for (const candidate of candidates) {
      ...
      if (result.action === null || result.action === 'flag_modify') {
        const c1 = buildC1(candidate, result, child_profiles, cleanNodeId, false);
        const alternatives = buildAlternatives(candidates, child_profiles, cleanNodeId, getActivityId(candidate));
        return res.status(200).json({ ...c1, alternatives, source: 'cloud_llm_proxy' });
      }

      if (result.action === 'block_and_substitute') {
        const substitute = selectSafeSubstitute(activityBank, cleanNodeId, getActivityId(candidate));
        if (substitute) {
          const result2 = GuardrailEngine.validate(substitute, child_profiles);
          const c1 = buildC1(substitute, result2, child_profiles, cleanNodeId, false);
    ...
    const fallback = buildStaticFallback(cleanNodeId, child_profiles);
    return res.status(200).json({ ...fallback, alternatives: [], source: 'cloud_llm_proxy' });
  } catch (err) {
    console.error('[/api/llm/activity] Error:', err.message);
    return res.status(500).json({ error: 'Internal server error.' });
  }
});
